...
const CafesPage = () => {
  ...
  const loadCafes = useCallback(async (loadMore = false) => {
  ...
  useEffect(() => { setOffset(0); setLoading(true); loadCafes(false); }, [search, sort, locationLat, locationLng]);
  ...
  const loadArchived = async () => {
    try {
      const res = await api.get('/cafes/archived');
  ...
  const handleRestore = async (id) => {
    try {
      await api.put(`/cafes/${id}/restore`);
      setArchivedItems(prev => prev.filter(i => i.id !== id));
      toast('Café restauré', 'success');
      loadCafes(false);
    } catch {
                ...
                {showArchived && archivedItems.length > 0 && (
                  <div className="mt-6">
                    ...
                    <div className="space-y-3">
                      {archivedItems.map(c => (
                        <div key={c.id} className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-xl opacity-60">
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-gray-700 dark:text-gray-300 truncate">{c.name}</p>
                            <p className="text-xs text-gray-400">{c.address || 'Pas d\'adresse'}</p>
                          </div>
                          <button
