...
const getAll = async (req, res) => {
  ...
  const countQuery = `SELECT COUNT(*) FROM cafes c WHERE c.is_archived = false ${searchClause}`;
...
const getArchived = async (req, res) => {
  try {
    const result = await db.query(`
      ...
      LEFT JOIN users u1 ON c.created_by = u1.id
      LEFT JOIN users u2 ON c.updated_by = u2.id
      WHERE c.is_archived = true AND c.created_by = $1
      ORDER BY c.updated_at DESC
    `, [req.user.id]);
    res.json(result.rows);
  } catch (err) {
    console.error('Get archived cafes error:', err);
    res.status(500).json({ error: 'Erreur lors de la récupération des cafés archivés' });
  }
};
...
const restore = async (req, res) => {
  try {
    const existing = await db.query(
      'SELECT created_by FROM cafes WHERE id = $1 AND is_archived = true', [req.params.id]
    );
    if (existing.rows.length === 0) {
      return res.status(404).json({ error: 'Café archivé introuvable' });
    }
    if (existing.rows[0].created_by !== req.user.id) {
      return res.status(403).json({ error: 'Non autorisé' });
    }
    ...
    const result = await db.query(
      'UPDATE cafes SET is_archived = false, updated_at = NOW() WHERE id = $1 RETURNING *',
      [req.params.id]
    );
    res.json(result.rows[0]);
  } catch (err) {
    console.error('Restore cafe error:', err);
