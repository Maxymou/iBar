...
const getAll = async (req, res) => {
  ...
  const countQuery = `SELECT COUNT(*) FROM restaurants r WHERE r.is_archived = false ${searchClause}`;
...
const update = async (req, res) => {
  ...
  try {
    const existing = await db.query(
      'SELECT photo_url, name, created_by FROM restaurants WHERE id = $1 AND is_archived = false', [id]
    );
...
const remove = async (req, res) => {
  try {
    const existing = await db.query(
      'SELECT created_by FROM restaurants WHERE id = $1 AND is_archived = false', [req.params.id]
    );
    if (existing.rows.length === 0) {
      return res.status(404).json({ error: 'Restaurant introuvable' });
    }
    ...
    await db.query(
      `UPDATE restaurants SET is_archived = true, updated_at = NOW() WHERE id = $1`,
      [req.params.id]
    );
...
const getArchived = async (req, res) => {
  try {
    const result = await db.query(`
      ...
      LEFT JOIN users u1 ON r.created_by = u1.id
      LEFT JOIN users u2 ON r.updated_by = u2.id
      WHERE r.is_archived = true AND r.created_by = $1
      ORDER BY r.updated_at DESC
    `, [req.user.id]);
    res.json(result.rows);
  } catch (err) {
    console.error('Get archived restaurants error:', err);
    res.status(500).json({ error: 'Erreur lors de la récupération des restaurants archivés' });
  }
};
...
const restore = async (req, res) => {
  try {
    const existing = await db.query(
      'SELECT created_by FROM restaurants WHERE id = $1 AND is_archived = true', [req.params.id]
    );
    if (existing.rows.length === 0) {
      return res.status(404).json({ error: 'Restaurant archivé introuvable' });
    }
    if (existing.rows[0].created_by !== req.user.id) {
      return res.status(403).json({ error: 'Non autorisé' });
    }
    ...
    const result = await db.query(
      'UPDATE restaurants SET is_archived = false, updated_at = NOW() WHERE id = $1 RETURNING *',
      [req.params.id]
    );
    res.json(result.rows[0]);
  } catch (err) {
